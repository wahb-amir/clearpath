...
const PATTERNS: Array<{
  ...
  },
  {
    ...
    regex:
      /\+\d{1,3}[\s-]?\d{2,4}[\s-]?\d{3,4}[\s-]?\d{2,4}|\(\d{2,4}\)[\s-]?\d{3,4}[\s-]?\d{2,4}/g,
    confidence: 0.85,
  },
  {
    type: "phone",
    ...
    regex:
      /\b(?:Phone|Tel|Mobile|Cell|Contact)\s*:?\s*(\d{3,4}[\s-]?\d{3,4}(?:[\s-]?\d{2,4})?)/gi,
    confidence: 0.75,
  },
  {
    type: "date",
...
const DEADLINE_KEYWORDS =
  /\b(deadline|due date|due by|must be submitted by|expires? on)\b/i;

function normalizeDate(raw: string): string | null {
  const parsed = new Date(
    raw.replace(/(\d{1,2})\/(\d{1,2})\/(\d{2,4})/, "$3-$2-$1"),
  );
